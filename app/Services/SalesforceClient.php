...
class SalesforceClient
{
    ...
    public function __construct()
    {
        ...
        $this->username  = trim((string) env('SF_USERNAME_SFDC', ''));
...
if (!$ok) {
    ...
    // 互換用: 旧API名を残しておく
	public function createDefect(array $payload)
	{
    	return $this->createClaim($payload);
	}

	// もし他にも旧名があれば同様に
	public function updateDefect(string $id, array $payload)
	{
    	return $this->updateClaim($id, $payload);
	}
	public function attachDefectImage(string $claimId, string $filename, string $mime, string $b64)
	{
    	return $this->attachClaimImage($claimId, $filename, $mime, $b64);
	}
    

    /**
     * アプリのフォーム値を受け取り、Salesforce項目にマッピングして Claim__c を作成
     * @param array $app Input keys:
     *  - occurred_at: string (datetime-local) 発生日時
     *  - severity: string 重大度（選択肢）
     *  - seiban: ?string 製番
     *  - office_name: ?string 装置名（営業所名）
     *  - overview: string 不具合内容（タイトル）
     * @return string Created Claim__c Id
     */
    public function submitClaimFromApp(array $app): string
    {
        // 発生日時 → ISO8601 UTC
        $occurredAt = $app['occurred_at'] ?? null;
        $dt = null;
        if ($occurredAt) {
            try {
                $dt = (new \DateTime($occurredAt, new \DateTimeZone(date_default_timezone_get())))
